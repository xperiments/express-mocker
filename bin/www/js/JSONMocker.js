    ...
    (function (xperiments) {
        ...
        (function (json) {
            ...
            var MicroMustache = (function () {
                ...
                MicroMustache.fixJsonArguments = function (json) {
                    return [
                        '"',
                        json.split(',').map(function (el) {
                            return (el.replace(/^\'(.*)\'$/, "\"$1\"").replace(/^\"(.*)\"$/, "$1").replace(/\"/g, '\\"'));
                        }).join('","'),
                        '"'
                    ].join('');
                };
                ...
                */
                function (template, view, partials) {
                    ...
                    return template.replace(/\{?\{\{\s*(.*?)\s*\}\}\}?/g, function (match, varName) {
                        ...
                        if (typeof partials[clearArgs] == "function" || typeof JSONMocker.templateObject[clearArgs] == "function") {
                            ...
                            var partialTarget = typeof partials[clearArgs] == "function" ? partials : JSONMocker.templateObject;
                ...
                return MicroMustache;
            })();

            var JSONMocker = (function () {
                function JSONMocker() {
                }
                JSONMocker.render = function (template, view) {
                    if (typeof view === "undefined") { view = {}; }
                    return MicroMustache.render(template, view, MicroJSONGenHelper);
                };

                JSONMocker.getObjectLength = function (object) {
                    return Object.keys(object).length;
                };

                JSONMocker.parseArray = function (array, count) {
                ...
                JSONMocker.parseVars = function (object) {
                    var repeat = /\$repeat\:(\d*)/;
                ...
                JSONMocker.parseObject = function (a) {
                ...
                JSONMocker.parseTemplate = function (templateObject, context) {
            ...
            json.JSONMocker = JSONMocker;

            var Injector = (function () {
                function Injector() {
                }
                Injector.process = function (target, returnCall) {
                    if (typeof returnCall === "undefined") { returnCall = true; }
                ...
                Injector.hasDependencies = function (target) {
                    var FN_ARGS = /^function\s*[^\(]*\(\s*([^\)]*)\)/m;
                    var FN_ARG_SPLIT = /,/;
                    var FN_ARG = /^\s*(_?)(\S+?)\1\s*$/;
                    var STRIP_COMMENTS = /((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg;
            ...
            var MicroJSONGenPlugin = (function () {
                function MicroJSONGenPlugin() {
                }
                MicroJSONGenPlugin.initialize = function () {
                };
                return MicroJSONGenPlugin;
            })();
            var MicroJSONGenHelper = (function () {
                function MicroJSONGenHelper() {
                }
                MicroJSONGenHelper.firstName = function () {
                    return MicroJSONGenHelper.rndFirst[MicroJSONGenHelper.random(MicroJSONGenHelper.rndFirst.length - 1)];
                };
                MicroJSONGenHelper.lastName = function () {
                    return MicroJSONGenHelper.rndLast[MicroJSONGenHelper.random(MicroJSONGenHelper.rndLast.length - 1)];
                };
                MicroJSONGenHelper.company = function () {
                    return MicroJSONGenHelper.rndCompanies[MicroJSONGenHelper.random(MicroJSONGenHelper.rndCompanies.length - 1)];
                };
                MicroJSONGenHelper.phone = function () {
                    return "+7095" + Math.random().toFixed(7).split(".")[1];
                };
                MicroJSONGenHelper.email = function () {
                    return (MicroJSONGenHelper.firstName() + "@" + MicroJSONGenHelper.company().replace(/[^a-zA-Z0-9]+/g, '') + ".com").toLowerCase();
                };
                MicroJSONGenHelper.bool = function () {
                    return Math.random() > .5 ? true : false;
                };
                MicroJSONGenHelper.random = function (random) {
                    return Math.floor(Math.random() * random);
                };
                MicroJSONGenHelper.query = function (id) {
                    return JSONMocker.context[id];
                };
                MicroJSONGenHelper.image = function (width, height) {
                ...
                MicroJSONGenHelper.guid = function () {
                ...
                MicroJSONGenHelper.numeric = function (e, n, r) {
                    var i = 0, s = (e + "").split(".").pop().length;
                    return e = +e, n = +n, e % 1 === 0 ? i = Math.round(e - .5 + Math.random() * (n - e + 1)) : i = parseFloat(Math.min(e + Math.random() * (n - e), n).toFixed(s)), +i;
                };

                MicroJSONGenHelper.lorem = function (paragraphs, wordsPerParagraph) {
                    if (typeof paragraphs === "undefined") { paragraphs = 1; }
                    if (typeof wordsPerParagraph === "undefined") { wordsPerParagraph = 100; }
                    function shuffle(o) {
        ...
        })(xperiments.json || (xperiments.json = {}));
        var json = xperiments.json;
